...
mod cli {
    ...
    pub(crate) fn overrides() -> &'static CliOverrides {
        CLI_OVERRIDES.get_or_init(CliOverrides::default)
    }
}
pub use cli::*;

// ── settings KV（运行时动态调参，落 SQLite） ──
...
mod settings {
    ...
    pub fn shadow_ratio(db: &crate::db::Db) -> f64 {
        setting(db, "routing.shadow_ratio")
            .and_then(|v| v.parse::<f64>().ok())
            .unwrap_or(0.10)
            .clamp(0.0, 1.0)
    }

    // P3 健康感知（ROUTING-PLAN §P3）：阈值全部走 settings KV，可在设置页运行时调整；
    // 默认值对应「滑窗 5 内 ≥2 失败降级、连续 3 失败宕机、连续 5 失败熔断」的自适应口径。

    /// 滑窗大小（保留最近 n 次 outcomes）。默认 5。
    pub fn health_fail_window(db: &crate::db::Db) -> i64 {
        setting(db, "health.fail_window")
    ...
    pub fn health_degraded_fails(db: &crate::db::Db) -> u32 {
        setting(db, "health.degraded_fails")
    ...
    pub fn health_down_consecutive(db: &crate::db::Db) -> u32 {
        setting(db, "health.down_consecutive")
            .and_then(|v| v.parse::<u32>().ok())
            .unwrap_or(3)
            .clamp(1, 10000)
    }
    ...
    pub fn health_circuit_threshold(db: &crate::db::Db) -> u32 {
        setting(db, "health.circuit_threshold")
            .and_then(|v| v.parse::<u32>().ok())
            .unwrap_or(5)
            .clamp(1, 10000)
    }
    ...
    pub fn health_probe_sec(db: &crate::db::Db) -> u64 {
        setting(db, "health.probe_sec")
...
mod paths {
    ...
    pub fn install_dir() -> PathBuf {
        PathBuf::from(std::env::var("LLOOM_INSTALL_DIR").unwrap_or_else(|_| ".".to_string()))
    }
...
mod net {
    ...
    pub(crate) fn web_port_with(over: &CliOverrides, web_port_env: Option<&str>) -> u16 {
        over.web_port
    ...
    pub(crate) fn bind_addr_with(over: &CliOverrides, bind_env: Option<&str>) -> String {
        over.bind
            .clone()
            .or_else(|| {
                bind_env
...
mod tests {
    ...
    fn web_port_precedence() {
        // 默认值
        assert_eq!(
            web_port_with(&CliOverrides::default(), None),
            DEFAULT_WEB_PORT
        );
        // env > 默认
    ...
    fn bind_addr_precedence() {
        ...
        assert_eq!(
            bind_addr_with(&CliOverrides::default(), Some("0.0.0.0")),
            "0.0.0.0"
        );
        let over = CliOverrides {
        ...
        // 空 env 视为未设置
        assert_eq!(
            bind_addr_with(&CliOverrides::default(), Some("  ")),
            "127.0.0.1"
        );
    }
}
