...
> **C1 附注**：为验证推理链路注册了 `deepseek-r1`（dashscope，已配真实单价 5.5e-7/2.2e-6 USD/token + reasoning_cost），保留在注册表中供 WebUI 思考折叠展示测试。
